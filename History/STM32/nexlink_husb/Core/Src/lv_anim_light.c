...
lv_anim_t* anim_head = NULL, *anim_tail = NULL;
...
void lv_anim_run()
{
  for(lv_anim_t* a = anim_head; a != NULL; a = a->next)
  {
    if(a->act_time > 0)
    {
      if(a->act_time > a->time)
    ...
    if(a->act_time <= a->time)
    {
      a->act_time +=  lv_tick_elaps(last_timer_run);
...
void lv_anim_start(lv_anim_t* a, uint32_t end, uint32_t duration)
{
	a->act_time = 0;
  a->time = duration;
  a->end_value = end;
...
void lv_anim_add(lv_anim_t* a, int32_t start, lv_anim_exec_xcb_t exec_cb)
{
  lv_anim_init(a);
  a->exec_cb = exec_cb;
  a->start_value = start;
