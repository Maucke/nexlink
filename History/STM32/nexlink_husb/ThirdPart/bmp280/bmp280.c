...
//	IIC_Write_Byte(BME280_ADDRESS,reg_address,data);

void BMP280_ReadTrim(void)
{
  uint8_t data[32];

  IIC_Read_Len(BME280_ADDRESS, 0x88, 24, &data[0]);
  IIC_Read_Len(BME280_ADDRESS, 0xA1, 1, &data[24]);
  IIC_Read_Len(BME280_ADDRESS, 0xE1, 7, &data[25]);
...
{
  u8 data[8];
  IIC_Read_Len(BME280_ADDRESS, 0xF7, 8, data);
...
void BMP280_Init(void)
{
  ...
  IIC_Init();		                                                           /*I2C*/
  IIC_Write_Byte(BME280_ADDRESS, 0xF2, ctrl_hum_reg);
  IIC_Write_Byte(BME280_ADDRESS, 0xF4, ctrl_meas_reg);
  IIC_Write_Byte(BME280_ADDRESS, 0xF5, config_reg);
  BMP280_ReadTrim();                    //
}
...
//    dbmsg("pressure: %f, temperature: %f, humidity: %f, asl: %f, altitude: %f m", pressure, temperature, humidity, asl, BMP280_PressureToAltitude(&pressure));
    dbmsg("alt: %.3f m", BMP280_PressureToAltitude(&pressure));
