...
//	if (i2c_request->length <= (64 - 8) && i2c_request->length <= (64 - 8))
//	{
//		if (i2c_request->length == 0 && i2c_request->length != 0) // Write Only
//		{
//			status = HAL_I2C_Master_Transmit(&hi2c1, i2c_request->deviceAddress << 1, i2c_request->dataWriteBuffer, i2c_request->length, i2c_request->timeout);
...
//			}
//		}
//		else if (i2c_request->length != 0 && i2c_request->length == 0) // Read Only
//		{
//			HAL_I2C_Master_Receive(&hi2c1, i2c_request->deviceAddress << 1, (uint8_t *)uI2c.data, i2c_request->length, i2c_request->timeout);
//			if (status != HAL_OK)
//			{
//				errorCode = HAL_I2C_GetError(&hi2c1);
//				uI2c.len = 2;
...
//			}
//			else
//				uI2c.len = i2c_request->length;
//		}
//		else if (i2c_request->length != 0 && i2c_request->length != 0) // WriteRead
//		{
//			status = HAL_I2C_Master_Transmit(&hi2c1, i2c_request->deviceAddress << 1, i2c_request->dataWriteBuffer, i2c_request->length, i2c_request->timeout);
//			// dbmsg("%X,%d,%d",i2c_request->deviceAddress << 1, i2c_request->length, i2c_request->timeout);
...
//		}
//		else
//			while (xQueueReceive(xQueue_UartData, &uUart, 10) == pdTRUE)
//			{
...
//	else
//	{
//		HAL_UART_Transmit_DMA(&huart1, (uint8_t *)uart_request->dataWriteBuffer, uart_request->length);
//		PutUartData(Tx, uart_request->dataWriteBuffer, uart_request->length);
//	}
}
